...
export function Formulario() {
  function prevenirEnvioFormulario(e) {
    ...
    if (datos[0].match(/^[A-Za-z]+$/)) {
      estado += "OK";
      msgNombre.innerHTML = "";
      msgNombre.className = msgNombre.style.backgroundColor = "white";
    } else {
    ...
    if (datos[1].match(/^[A-Za-z]+$/)) {
      ...
      msgApellido.className = msgApellido.style.backgroundColor = "white";
    } else {
      msgApellido.className = "alert alert-danger";
      msgApellido.innerHTML = "Apellido no es valido";
    }

    if (datos[2].match(/^[0-9]+$/)) {
      estado += "OK";
      msgCedula.innerHTML = "";
      msgCedula.className = msgCedula.style.backgroundColor = "white";
    } else {
      msgCedula.className = "alert alert-danger";
      msgCedula.innerHTML = "Cedula no es valido";
    }

    if (datos[3].match(/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/)) {
      estado += "OK";
      msgCorreo.innerHTML = "";
      msgCorreo.className = msgCorreo.style.backgroundColor = "white";
    } else {
    ...
    if (datos[4].match(/^[#.0-9a-zA-Z\s,-]+$/)) {
      estado += "OK";
      msgDireccion.innerHTML = "";
      msgDireccion.className = msgDireccion.style.backgroundColor = "white";
    } else {
    ...
    if (datos[5].match(/^[0-9]+$/)) {
      ...
      msgTelefono.className = msgTelefono.style.backgroundColor = "white";
    } else {
    ...
    if (datos[6].match(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[^\W_]{8,}$/)) {
      estado += "OK";
      msgContra1.innerHTML = "";
      msgContra1.className = msgContra1.style.backgroundColor = "white";
    } else {
